...
- **2026-09-18 — Fase 5.1 (dual-env transparente: UI x64 + helper 32-bit) ✅:**
  - `hardware/henry7x/helper_main.py` (novo, sem UI): argparse `--mock/--transport stdio|tcp/--host/--port/--dll-path/--giro-delay-s`; loop JSON lines (stdio ou TCP `127.0.0.1:9477`); cmds `conectar/liberar/bloquear/desconectar/status/ping/quit` + eventos espontâneos `{"event":"giro",...}` via callback do driver; driver preguiçoso (Real ou Mock); sem DLL o helper sobe e responde `ok:false` (falha graciosa); log só em stderr.
  - `hardware/henry7x/helper_client.py` (novo): `Henry7xHelperClient(Henry7xDriver)`, `is_mock=False`; resolve exe (`GYMFLUX_HELPER_EXE` > ao lado de `sys.executable` > `_MEIPASS` > `GYMFLUX_HELPER_DIR`; `cmd` explícito p/ testes); `Popen` stdio + thread leitora (respostas por id, giros → callbacks); spawn sob demanda, `desconectar()` envia `quit` e encerra (sem órfãos, `CREATE_NO_WINDOW` no win); `conectar` propaga `RuntimeError`, `liberar`→ERRO em falha, `status` nunca levanta (+`via_helper`). NUNCA importa `win32com`.
  - `factory.py`: win32 64-bit → tenta `Henry7xHelperClient`, sem helper → Mock+warning; win32 32-bit → `RealHenry7x` como antes; Linux → Mock. `__init__` exporta `Henry7xHelperClient`.
  - Specs: `gymflux.spec` x64 (hidden +`helper_client`, win32com/comtypes removidos e em `excludes`); novo `henry_helper.spec` (onefile console `GymFlux.HardwareHelper`, hidden win32com/comtypes/loguru, excludes PySide6/sqlalchemy/pydantic/testes, `datas=[]`); `.gitignore` +`!henry_helper.spec`.
  - `installer/gymflux.iss`: `[Files]` +`dist\GymFlux.HardwareHelper.exe` (sem atalho, só `GymFlux.exe` tem ícones); `scripts/build.ps1/.bat` dual (UI x64 no `.venv` + helper x86 em `.venv32` via `UV_PROJECT_ENVIRONMENT`, detecta `py -3.11-32`/`GYMFLUX_PYTHON_X86`, fallback só avisa); `release.yml` 3 jobs (`build-helper` x86 sem extra ui + `build-ui` x64 + `package` com artifacts e iscc v1.2.9), timeout 25min.
  - Testes: novo `tests/unit/hardware/test_helper_ipc.py` (6 testes: resolve cmd, roundtrip mock via subprocess, evento giro, falha graciosa sem DLL, factory win64 simulada sem `win32com`); `test_real_henry.py` aceita helper em win64 (nunca Real, sem `win32com`).
  - Verificação Linux: ruff/mypy limpos, `uv run pytest -q` **203 passed +1 skipped**, `gymflux --help` ok. **core/services/infra/models/UI intocados.**
  - `kernel7x.dll` NÃO bundlada (helper falha gracioso sem `regsvr32`).
- **2026-09-18 — Fase 5.2 (resiliência catraca) ✅:**
  - `ui/views/dashboard.py`: `DetalhesDialog(bridge)` (aceita dict legado sem retry) + tbl + `[Tentar reconectar|Fechar]`; retry em thread com spinner textual, recarrega tbl, erro em vermelho (`Último problema/Detalhe/Atenção`), callback `on_reconnect` → `lbl_compacto` verde + toast; auto-retry no polling 1s (3 tentativas, 2s, thread via `reconnect_concluido`, sem travar UI); `sincronizar_bridge()` pós hot-swap.
  - `ui/config_store.py`: `UiConfig.modo_catraca: Literal["real","mock"]` default `"real"` (JSON legado sem chave → `"real"`, roundtrip ok); `views/config.py` combo "Equipamento real/Demonstração (mock)" + toast "Reinicie o app..." ao trocar Real↔Mock.
  - `hardware/henry7x/factory.py`: `prefer_mock None` → env `GYMFLUX_HENRY_MOCK` > `UiConfig.modo_catraca` explícito (`_modo_catraca_configurado()`; None se ausente/legado) > `settings.henry_mock`; `ui/catraca_bridge.py` +`trocar_driver()` (hot-swap + reconecta); `ui/app.py` `_aplicar` troca driver ao mudar modo + `sincronizar_bridge()` nas views.
...
- **2026-09-18 — Fase 5.3 (toast overlay + ícones win + scroll Config) ✅:**
  ...
  - Auditoria gerente 2026-09-18: toast overlay fora de layout (não estica, WA_TransparentForMouseEvents, resize só move), ícones 16px no win (theme.py Qt-free, header intacto), Config com scroll; 216 passed confirmam. APROVADO.
- **2026-09-18 — Fase 5.4 (widgets modernos + ícones win DPI + DB sempre em APPDATA) ✅:**
  - Widgets (`ui/theme.py`, QSS global ambos modos): `QTextEdit`/`QDateEdit`/`QTimeEdit` na base + `:focus`/`:disabled`; setas flat (`::up-button`/`::down-button`/`::drop-down` sem borda, hover `#5AC8FA`, 18/22px, sem SVG externo); `funcionarios.py` sem QSS inline de senha p/ remover (só busca transparente intencional).
  - Ícones (`ui/theme.py` +`normalizar_icone()`, Qt-free no import): `icone_preto`/`icone_vermelho` devolvem pixmap único em `icon_size()` (cache por tamanho); `dashboard.btn_liberar` embrulhado (tabs via `app._tint_icon` intactas). Baseline Linux: multisize 16/32/128, `actualSize(16)==16`.
  - DB (`ui/app.py:_ensure_schema`): mkdir APPDATA antes + exporta `GYMFLUX_DB_URL` p/ `%APPDATA%` em frozen (`env.py` prefere settings e ignorava o override do ini — causa raiz do e9df8ef não pegar) + fallback `init_db()` se upgrade falhar (nunca propaga); `modo_catraca` não influencia DB. `infra/db.py` inalterado (todos os caminhos já via `get_default_db_url()`).
  - Testes: `test_db_frozen.py` (3: `init_db` cria `.db`+tabelas em APPDATA simulado, mapping de URL, `_ensure_schema` slow sem catraca/modo real), `test_icones_dpi.py` (2: win32 simulado `actualSize==16`+`sizeHint<220`, nulo/Linux), QSS texto (7 seletores ×2 modos) + smoke render claro/escuro.
  - Verificação Linux: ruff/mypy limpos, `uv run pytest -q` **223 passed +1 skipped** (216 +7). **core/services/infra/models intocados.**
  - Resgate stash "temp theme" (commit separado `69ad237`): `real.py` validado Windows (ProgID `Kernel7x.Kernel` + fallbacks, SComConfig aninhado 7.2, comtypes best-effort) + `test_progid` + `dump_henry_typelib.py` + `.env.example` prod; stashes `wip 5.4`/`wip antes 5.3`/`temp theme` resolvidos (ícone 285K restaurado, resto superseded/dropado, lista limpa).
- **2026-09-18 — Fase 5.4-B (setas visíveis + botão senha com ícone) ✅:**
  - Setas (`ui/theme.py`): removidas regras `::-arrow` só com tamanho (escondiam a seta nativa no Windows); botões com `subcontrol-origin/position` + `margin:1px` + hover `#5AC8FA`; seta nativa sempre visível. Screenshots `/tmp/setas_{claro,escuro}.png`.
  - Senha (`ui/views/funcionarios.py` +`_botao_ver_senha()`): QToolButton `VerSenha` com QIcon (aberto `SP_FileDialogContentsView` preto / fechado `SP_DialogCancelButton` vermelho, `icon_size()`, sem emoji) + estilo flat global `QToolButton#VerSenha` (ambos modos); aplicado em Novo + Perfil (Perfil ganha HBox; desabilitado em read_only).
  - Testes: QSS sem `::-arrow` (×2 modos), botão senha (ícone/IconSize/toggle echo), resto intacto.
  - Verificação Linux: ruff/mypy limpos, `uv run pytest -q` **225 passed +1 skipped** (223 +2). **core/services/infra/models intocados.**
...
- Factory `get_henry_driver()` decide por `sys.platform` + env var `GYMFLUX_HENRY_MOCK=1`. Em prod o COM roda no helper via `win32com.client.Dispatch("Henry.Kernel7x")`, nunca no processo UI (`ctypes` proibido em todo `src/`).
- Resiliência Fase 5.2: `DetalhesDialog` reconecta via bridge (thread + spinner, erro em vermelho); polling agenda auto-retry 3x/2s offline; `UiConfig.modo_catraca` (`real`/`mock`, legado→`real`) lido pela factory (env > config explícita > settings); troca Real↔Mock na Config faz hot-swap via `bridge.trocar_driver()` + toast de reiniciar.
- Dual-env Fase 5.1 (1 app só): `GymFlux.exe` (x64, PySide6) + `GymFlux.HardwareHelper.exe` (x86, COM oculto) no mesmo `{app}`; a UI acha o helper ao lado de `sys.executable`/`sys._MEIPASS` (`GYMFLUX_HELPER_EXE` faz override, `GYMFLUX_HELPER_DIR` adiciona candidato). Sem helper em win64 → Mock + warning (UI abre normal).
- Processo x64 NUNCA importa `win32com` (só o helper importa); protocolo JSON lines em stdio (TCP `127.0.0.1:9477` opcional via `--transport tcp`); `desconectar()` do cliente encerra o helper (sem órfãos).
- Build Windows: `pwsh scripts/build.ps1` (UI x64 no `.venv` + helper x86 em `.venv32` via `UV_PROJECT_ENVIRONMENT`; detecta `py -3.11-32` ou `GYMFLUX_PYTHON_X86`, sem x86 só avisa). CI: jobs `build-helper` (x86, `--extra windows` SEM `ui` — PySide6 só tem wheels win_amd64) + `build-ui` (x64) + `package` (iscc).
...
---
*Última atualização: 2026-09-18 por subordinado Fase 5.4-B (setas visíveis + botão senha ícone, 225 passed). Mantenha este arquivo enxuto e factual.*
