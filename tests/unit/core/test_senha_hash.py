...
Roda com ``GYMFLUX_PBKDF2_ITERATIONS=1000`` via fixture autouse em
...
from gymflux.core import aluno as aluno_mod
from gymflux.core.aluno import (
    _PBKDF2_ITERACOES,
    _iteracoes_pbkdf2,
    conferir_senha,
    gerar_senha_hash,
...
def test_default_prod_mantem_100_mil(monkeypatch):
    ...
    monkeypatch.delenv("GYMFLUX_PBKDF2_ITERATIONS", raising=False)
    assert _iteracoes_pbkdf2() == 100_000
    monkeypatch.setenv("GYMFLUX_PBKDF2_ITERATIONS", "lixo")
    assert _iteracoes_pbkdf2() == 100_000


def test_env_configura_iteracoes_e_hash_carrega_contagem(monkeypatch):
    monkeypatch.setenv("GYMFLUX_PBKDF2_ITERATIONS", "1000")
    ...
    assert aluno_mod._ENV_PBKDF2_ITERACOES == "GYMFLUX_PBKDF2_ITERATIONS"
