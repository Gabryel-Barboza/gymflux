...
from gms_app.hardware.henry7x.interface import Henry7xDriver


def get_henry_driver(prefer_mock: bool | None = None) -> Henry7xDriver:
    """Retorna driver adequado.

    - Se GMS_HENRY_MOCK=1 ou prefer_mock=True -> MockHenry7x
    ...
    env_mock = os.getenv("GMS_HENRY_MOCK")
    if prefer_mock is None:
        prefer_mock = env_mock == "1" if env_mock is not None else settings.henry_mock

    if prefer_mock:
        logger.info("Factory: usando MockHenry7x (GMS_HENRY_MOCK=1)")
        from gms_app.hardware.henry7x.mock import MockHenry7x
    ...
    try:
        from gms_app.hardware.henry7x.real import RealHenry7x

        dll_path = os.getenv("GMS_HENRY_DLL_PATH", settings.henry_dll_path)
