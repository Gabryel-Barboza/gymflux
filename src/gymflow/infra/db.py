"""Engine + Base + Session — SQLAlchemy 2.0 SQLite GymFlow.

- URL vem de GYMFLOW_DB_URL (default sqlite:///data/gymflow.db).
...
def get_engine(db_url: str | None = None, echo: bool = False) -> Engine:
    ...
    if url.startswith("sqlite"):
        ...
        if ":memory:" not in url:
            try:
                # extrai path p/ sqlite:///data/gymflow.db ou sqlite:////abs/path
...
def init_db(db_url: str | None = None, echo: bool = False) -> Engine:
    ...
    from gymflow.infra import models as _models  # noqa: F401
