...
class FuncionariosViewModel:
    ...
    def cadastrar(self, *, nome: str, senha: str) -> Funcionario:
        func = Funcionario(id=f"func-{uuid.uuid4().hex[:8]}", nome=nome.strip())
        func.definir_senha(senha)  # ValueError se fora de 4-8 dígitos
        self.repo.salvar(func)
        self._commit()
        return func

    def atualizar(self, funcionario_id: str, *, nome: str, senha: str | None = None) -> Funcionario:
